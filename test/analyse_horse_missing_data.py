    ...
    for mask in dp_masks:
        for n in percent_samples_observed:
            for d in percent_dimensions_observed:
                ...
                # n = 0.8  # 0.7, 0.9
                # d = 0.7  # 0.8, 0.9
                desired_configs = ['walk_train_seed_{0}_mask_{1}_'.format(i, mask) +
                                   'n_percent_{0}_d_percent_{1}.npz'.format(n, d) for i in seeds]
                desired_bgplvm_configs = ['bgplvm_horse_missing_data_f_star_' + i for i in desired_configs]
                # desired_mrd_configs = ['mrd_horse_missing_data_' + i for i in desired_configs]
                desired_gpdp_configs = ['gpdp_horse_missing_data_f_star_' + i for i in desired_configs]
