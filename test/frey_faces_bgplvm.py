...
def run_bgplvm(y_train, y_test_observed, y_test_unobserved, num_latent_dimensions, num_inducing_points,
    ...
    print('  SNR (dB): {}'.format(10.0 * np.squeeze(np.log10(signal_variance * noise_precision))))


if __name__ == '__main__':

    # Optimisation variables.
    learning_rate = 0.05  # 0.025  # 0.01  # 0.05
    num_iter_train = 2500
    ...
    for s in data_seeds:
        ...
        test_indices = training_indices[:num_test_samples]  # Predict training data.

        # Normalise data to zero mean and unit variance.
        scaler = MinMaxScaler(feature_range=(0.0, 1.0), copy=True)
        ...
        # std_devs = np.std(training_data, axis=0, ddof=1)
        # print('Max standard deviation: {}'.format(np.max(std_devs)))
        # print('Min standard deviation: {}'.format(np.min(std_devs)))
        # index = np.argmin(std_devs)
        # print('Index of min standard deviation: {}'.format(index))
        # plot.hist(training_data[:, index])
        # plot.show()
        # quit(0)
        ...
        seed_val = 10
        dataset_str = 'frey_faces_50_missing_data__predict_training_seed{}_n{}_m{}_q{}_init_seed{}'.format(
            s, num_training_samples, num_inducing_points, num_latent_dimensions, seed_val)
        ...
        if show_plots or save_plots:
            ...
            if permuted_data:
                ...
                bgplvm_scaled_predicted_images = bgplvm_permuted_predicted_images[:, inverse_indices]
                bgplvm_predicted_images = scaler.inverse_transform(bgplvm_scaled_predicted_images)
            else:
                bgplvm_predicted_mean = np.load(bayesian_gp_lvm_results_file)['predicted_mean']
                bgplvm_predicted_images = scaler.inverse_transform(np.hstack(
                    (test_data[:, :num_observed_dimensions], bgplvm_predicted_mean)))
            # assert ground_truth.shape[0] == predicted_images.shape[0]
            for i in range(ground_truth.shape[0]):
                # plot.figure()
                fig_size = (4, 3)  # (10, 5)
                fig, (ax1, ax2) = plot.subplots(nrows=1, ncols=2, sharey='row', figsize=fig_size)
